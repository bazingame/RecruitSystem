...
if($info_opt=='insertInfo'){
    ...
    if( isset($_POST['name']) && isset($_POST['sex']) && isset($_POST['birthday']) && isset($_POST['college']) && isset($_POST['profess']) && isset($_POST['phone']) && isset($_POST['qq']) && isset($_POST['introduction']) && $_POST['name'] != '' && $_POST['sex'] != '' && $_POST['birthday'] != '' && $_POST['college'] != '' && $_POST['profess'] != '' && $_POST['phone'] != '' && $_POST['qq'] != '' && $_POST['introduction'] != ''){
        ...
        if($match){}else{
            $result = array('code'=>'-3','msg'=>'姓名格式有点问题哇！');
        ...
        if($match){}else{
            $result = array('code'=>'-3','msg'=>'姓别格式有点问题哇！');
            $result = json_encode($result,JSON_UNESCAPED_UNICODE);
            die($result);
        }

        $birthday = htmlspecialchars($_POST['birthday'], ENT_QUOTES);
        if($org_key=='xtuart'){}else{
            $match = preg_match('/^\d{4}-\d{2}-\d{2}$/',$birthday);
            if($match){}else{
                $result = array('code'=>'-3','msg'=>'生日格式有点问题哇！');
                $result = json_encode($result,JSON_UNESCAPED_UNICODE);
                die($result);
            }
        }
        ...
        if($match){}else{
            $result = array('code'=>'-3','msg'=>'学院格式有点问题哇！');
            $result = json_encode($result,JSON_UNESCAPED_UNICODE);
            die($result);
        }
...
//        if($match){}else $pass=0;
        ...
        $match = preg_match('/^1[3|4|5|7|8][0-9]{9}$/',$phone);
        if($match){}else{
            $result = array('code'=>'-3','msg'=>'手机号格式有点问题哇！');
            $result = json_encode($result,JSON_UNESCAPED_UNICODE);
            die($result);
        }
        ...
        if($match){}else{
            $result = array('code'=>'-3','msg'=>'qq格式有点问题哇！');
            $result = json_encode($result,JSON_UNESCAPED_UNICODE);
            die($result);
        }
...
//    获取组织报名表信息
    ...
    $result = json_encode($result,JSON_UNESCAPED_UNICODE);
    echo $result;
}else if($info_opt=='dep_list'){
    $org_key = $_GET['org_key'];
    $res = $DB->getDepListArr($org_key);
